    ...
    >
      <div className="relative h-64 w-full bg-gray-50/50 overflow-hidden">
        ...
        <Image
          ...
          alt={product.name}
          fill
          className={`object-contain p-6 transition-all duration-700 ease-in-out ${isHovered && hasSecondaryImage ? "opacity-0 scale-95" : "opacity-100 scale-100"
            }`}
        ...
        {hasSecondaryImage && (
          <Image
            ...
            fill
            className={`object-contain p-6 transition-all duration-700 ease-in-out absolute inset-0 ${isHovered ? "opacity-100 scale-110" : "opacity-0 scale-100"
              }`}
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw"
          />
        )}
      ...
      <div className="p-6 flex flex-col grow">
        ...
        <button
          ...
          className={`w-full mt-6 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all active:scale-95 
            ${isOutOfStock
